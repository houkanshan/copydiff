...
const renderLine = (line: DiffLine): string => {
  const html = ansiToHtml(line.rawAnsi);
...
const renderHunk = (lines: DiffLine[], foldSegments: FoldSegmentMap): string => {
  const output: string[] = [];
  let index = 0;
  while (index < lines.length) {
    const fold = foldSegments.get(index);
    if (fold) {
      const foldedLines = lines.slice(index, fold.endIndex + 1).map((line) => renderLine(line)).join("\n");
      output.push(
...
const renderHtml = (files: FileDiff[], options: HtmlRenderOptions): string => {
  ...
  files.forEach((file) => {
    file.headerLines.forEach((line) => content.push(renderLine(line)));
    file.hunks.forEach((hunk) => {
      content.push(renderLine(hunk.headerLine));
      ...
      content.push(renderHunk(hunk.lines, foldMap));
    });
  });
